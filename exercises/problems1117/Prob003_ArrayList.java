...
public class Prob003_ArrayList {
	...
	public static void prnDisplay(ArrayList<Kospi> aList){
		...
		for(Kospi data: aList) {
			String fluctuation = data.getFluctuation();
			int open = fluctuation.indexOf("[");
			int close = fluctuation.indexOf("]");
			
			if(!fluctuation.substring(open, close).contains("-"))
				System.out.println(data.getFluctuation());
